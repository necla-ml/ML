...
class DataLoader(torch.utils.data.dataloader.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, nbatches=0, shuffle=False, sampler=None, batch_sampler=None,
        ...
        super(DataLoader, self).__init__(dataset, batch_size, shuffle, sampler, batch_sampler, num_workers, collate_fn, pin_memory, drop_last)

        # XXX custom
        self.src = dataset
        self.nbatches = min(nbatches, int(math.ceil(len(dataset) / batch_size))) if nbatches > 0 else int(math.ceil(len(dataset) / batch_size))
        ...
        if batch_sampler is not None:
            if batch_size > 1 or shuffle or sampler is not None or drop_last:
                raise ValueError('batch_sampler is mutually exclusive with '
                                 'batch_size, shuffle, samplerCls, and drop_last')
    ...
    def __iter__(self):
        ...
        size = self.batch_size * self.nbatches if self.drop_last else min(self.batch_size * self.nbatches, len(self.src) - self.offset)
        ...
        #nbatches = (len(self.dataset) + self.batch_size - 1) // self.batch_size
        #print(f"len(src)={len(self.src)}, len(dataset)={len(self.dataset)}, len(self.sampler)={len(self.sampler)}, offset={self.dataset.offset}, size={self.dataset.size}, nbatches={nbatches}")

        #itr = DataLoaderIter(self)
    ...
    def __len__(self):
        return self.batch_sampler and len(self.batch_sampler) or (len(self.dataset) + self.batch_size - 1) // self.batch_size
