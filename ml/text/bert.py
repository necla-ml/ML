...
def tokenize(text, plain=True, max_tokens=None):
    ...
    ), "setup(base, uncased) must be called to initailize the BERT tokenizer first"
    ...
    if max_tokens is not None:
        ...
        while True:
            ...
            total = sum(lengths)
            if total > max_tokens:
                sents[th.argmax(lengths).item()].pop()
            else:
                break
...
def tensorize(tokens, max_tokens=None, device=th.device("cpu")):
    ...
    if L <= max_tokens:
        ...
        logger.warn(f"Truncating length from {L} to {max_tokens}")
        padding = []
    ...
    return (
        th.tensor(tokens, device=device),
        th.tensor(segment, device=device),
        th.tensor(mask, device=device),
    )
...
def convert_snli(batch, device=th.device("cpu")):
    segments = []
    masks = []
    ...
    return (
        ...
        th.tensor(masks, device=device),
        th.tensor(segments, device=device),
...
def save(model, path):
    ...
    th.save(model_to_save.state_dict(), output_dir / WEIGHTS_NAME)
